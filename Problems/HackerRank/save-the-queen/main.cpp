...
int solve(int h, vector<int> wallPoints, vector<int> lengths) {
    const int posterCount = wallPoints.size();

    double minLeaderHeight = 0.f;

    for (int p = 0; p < posterCount; p++) {
        const double holdingPoint = wallPoints[p] - lengths[p] * 0.25;
        minLeaderHeight = max(minLeaderHeight, ceil(holdingPoint - h));
    }

    return minLeaderHeight;
}
